...
type feature
  real(kind=8),allocatable:: feat_data(:,:)
...
type feature_selected
  ...
  real(kind=8),allocatable:: feat_data(:,:),feat_score(:,:)
     ...
     ! broadcast ntot-nthis
      ...
      if(mpirank==0) then  ! send nthis 
        do i=2,mpisize
          ...
          if(mpin(i)>0) then
          call mpi_send(inp%feat_data(:,j:ntot),k*npoint,mpi_double_precision,i-1,31,mpi_comm_world,mpierr)
          call mpi_send(inp%feat_name(j:ntot),k*str_len,mpi_character,i-1,32,mpi_comm_world,mpierr)
          call mpi_send(inp%lastop(j:ntot),k*10,mpi_character,i-1,33,mpi_comm_world,mpierr)
          call mpi_send(inp%feat_comp(j:ntot),k,mpi_integer8,i-1,34,mpi_comm_world,mpierr)
          call mpi_send(inp%feat_unit(:,j:ntot),k*nunit,mpi_double_precision,i-1,35,mpi_comm_world,mpierr)
          end if
        end do
      else  ! receive
      ...
      if(mpin(mpirank+1)>0) &
      call combine(inp,i,j,1_8,k,trim(adjustl(ops(icomb))),nf(icomb),total_comb)
  ...
  IF (icomb < rung) THEN
      ...
      !--------------------------------------------------------------------
       ...
       if(mpirank>0 .and. i>0) then
          call mpi_send(gen%feat_data(:,:i),npoint*i,mpi_double_precision,0,2,mpi_comm_world,mpierr)
          call mpi_send(gen%feat_name(:i),i*str_len,mpi_character,0,3,mpi_comm_world,mpierr)
          call mpi_send(gen%lastop(:i),i*10,mpi_character,0,4,mpi_comm_world,mpierr)
          call mpi_send(gen%feat_comp(:i),i,mpi_integer8,0,5,mpi_comm_world,mpierr)
          call mpi_send(gen%feat_unit(:,:i),nunit*i,mpi_double_precision,0,6,mpi_comm_world,mpierr)
...
type(feature) myinp
real progress
real(kind=8) data_tmp(ubound(myinp%feat_data,1)),unit_tmp(ubound(myinp%feat_unit,1))
...
real(kind=8) feat_data(:),feat_unit(:),scoretmp(2),maxabs
...
real(kind=8) dim1(:),dim2(:),dimcomb(ubound(dim1,1))
character(len=*) op
integer i,j,k
...
integer(kind=8) simpler,complexity1,complexity2
...
do while( any(pavailable) .and. i<nf_sis(iFCDI) )
      ...
      if((loc(1)-1)==mpirank) then
         if(mpirank==0) then
            ...
            call mpi_send(sel%feat_data(:,loc(2)),npoint,mpi_double_precision,0,100,mpi_comm_world,mpierr)
...
real(kind=8) feat(:),sdfeat(ubound(feat,1)),tmp(ntask),sis_score(2),yyy(:),xnorm(ntask),xmean(ntask),&
...
real(kind=8) feat_data(:),feat_unit(:)
...
real(kind=8) tmpf(ubound(sel%feat_data,1),nbasic_select),tmpscore(2,nbasic_select)
...
! -1=< ffcorr <= 1
real(kind=8) ffcorr,feat1(:),feat2(:),mean1,mean2,sd1,sd2
...
real(kind=8) score1,score2,diff1,diff2,feat1(:),feat2(:)
logical equivalent
